...
const CURRENT_SYSTEM = room || "frontend";
...
export function ConnectToBackend() {
  SOCKET = io.connect(CONFIG_BACKEND_URL, {
    query: { system: CURRENT_SYSTEM },
  }); // make the backend know we're sending communicating from frontend
  var ADDED_LISTENERS = false;

  SOCKET.on("connect", () => {
    // clear map once we connect, so any previous data is wiped

    if (ADDED_LISTENERS) return;
    ...
    SOCKET.on(SocketEvent.JOIN, (packet) => {
      ...
      if (player == null) ConnectPlayer(packet);
    });
    ...
    SOCKET.on(SocketEvent.UPDATE, (packet) => {
      ...
      if (player == null) player = ConnectPlayer(packet);
      else {
        // update state
...
function ClearNetworkedActors() {
  for (let [key, value] of PLAYERS) DeleteObject(value);

  for (let [key, value] of PLAYERS) PLAYERS.delete(key);
}
